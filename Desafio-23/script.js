  ...
  */
  ...
  function handleClickEqual(event){
  	...
  	$visor.value = allValues.reduce(function(accumulated, current){
  		...
  		var nextValue = removeLastItemIfItIsAnOperation(current);
  		var nextOperator = isLastItemAnOperation(current) ? current.split('').pop() : '';
  		switch(operator){
  			case "+": return ( Number(firstValue) + Number(nextValue) ) + nextOperator;
  			case "-": return ( Number(firstValue) - Number(nextValue) ) + nextOperator;
  			case "x": return ( Number(firstValue) * Number(nextValue) ) + nextOperator;
  			case "÷": return ( Number(firstValue) / Number(nextValue) ) + nextOperator;
  		}
  	});
  }
